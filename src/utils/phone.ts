// ============================================================================
// MB CRUNCHY — Indian Phone Number Normalization & Validation (Frontend)
// ============================================================================
//
// Canonical internal format: +91XXXXXXXXXX
//   where XXXXXXXXXX is exactly 10 digits starting with 6, 7, 8, or 9.
//
// This utility is duplicated in convex/utils/phone.ts for the backend.
...
/**
 * Extract the raw 10-digit portion from user input for display in the
 * +91 | XXXXXXXXXX input component. Returns empty string if invalid.
 */
export function extractDigitsForInput(input: string): string {
  const digits = extract10Digit(input);
  return digits ?? "";
}
