  ...
  return (
    <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center">
      <div className="relative min-w-0 flex-1">
        ...
        <Input value={filters.query} onChange={(event) => onFiltersChange({ ...filters, query: event.target.value })} placeholder="Search by name or slug…" aria-label="Search products" className="pl-9" />
      </div>
      <div className="flex gap-2">
        <Select value={filters.businessUnitId} onValueChange={(value) => onFiltersChange({ ...filters, businessUnitId: value as ProductFilters["businessUnitId"] })}>
        ...
        <Select value={filters.status} onValueChange={(value) => onFiltersChange({ ...filters, status: value as ProductStatus | "all" })}>
