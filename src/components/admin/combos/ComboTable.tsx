...
export function ComboTable({ combos, isLoading = false, sortKey, sortDirection, onSort, onEdit, onDelete, onRestore }: ComboTableProps) {
  return <Table>
    ...
    <TableBody>
      {isLoading ? Array.from({ length: 6 }, (_, index) => <TableRow key={index}><TableCell><Skeleton className="size-9" /></TableCell>{Array.from({ length: 8 }, (_, cellIndex) => <TableCell key={cellIndex}><Skeleton className="h-5 w-24" /></TableCell>)}</TableRow>) : combos.map((combo) => { const highlightBadge = typeof combo.settings?.highlightBadge === "string" ? combo.settings.highlightBadge : undefined; return (
