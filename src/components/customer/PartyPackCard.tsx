...
export const PartyPackCard = memo(function PartyPackCard({
  ...
  const price = partyPack.price;
  const compareAtPrice = partyPack.compareAtPrice;
  const discount = compareAtPrice ? calculateDiscount(price, compareAtPrice) : 0;
  const itemCount = partyPack.items?.reduce((sum, item) => sum + item.quantity, 0) ?? 0;
    ...
    >
      ...
      >
        <div className="flex flex-col sm:flex-row">
          {/* Image Section */}
          <div className="relative aspect-[4/3] sm:aspect-square sm:w-52 shrink-0 overflow-hidden bg-secondary">
            {coverSrc && !imageError ? (
              <img
                src={coverSrc}
                alt={partyPack.name}
            ...
            <div className="absolute bottom-2 left-2 right-2">
              <Badge
                variant="secondary"
                className="gap-1.5 bg-background/80 backdrop-blur-sm text-foreground text-[10px] font-medium"
              >
                <Users className="h-3 w-3" />
                {partyPack.minServings}–{partyPack.maxServings} servings
              </Badge>
            </div>
          </div>
          ...
          <div className="flex flex-1 flex-col p-4">
            ...
            {/* Items List */}
            {partyPack.items && partyPack.items.length > 0 && (
              <div className="mt-3 space-y-1">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                  Includes ({itemCount} items)
                </p>
                <ul className="space-y-1">
                  {partyPack.items.slice(0, 4).map((item, i) => (
                    <li
...
<span className="truncate">
                  ...
                  {partyPack.items.length > 4 && (
                    <li className="text-[10px] text-muted-foreground">
                      +{partyPack.items.length - 4} more items
                    </li>
                  )}
                </ul>
              </div>
 ...
 */
export function PartyPackCardSkeleton() {
  return (
    <Card>
      <div className="flex flex-col sm:flex-row">
        <div className="aspect-[4/3] sm:aspect-square sm:w-52 animate-pulse bg-secondary" />
